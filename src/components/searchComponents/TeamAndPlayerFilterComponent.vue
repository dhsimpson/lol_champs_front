...
export default {
    ...
    setup() {
        const {showModal, currModalName, setModalName, isShowModal} = atomicModal('teamPlayerFilterModal');
        return {showModal, currModalName, setModalName, isShowModal};
    },
    methods: {
        selectedTeamAndPlayer() {
            let teamAndPlayer = this.params.team;
            if(this.params.player && this.params.player != PLAYER_NOT_SELECTED) {
                teamAndPlayer += `>${this.params.player}`;
            }
